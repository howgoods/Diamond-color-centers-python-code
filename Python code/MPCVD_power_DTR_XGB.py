...
def load_and_split_data(path, test_size=0.3, seed=42):
    df = pd.read_csv(path)
    X = np.around(np.array(df), 3)[..., 0:2]
    y = np.around(np.array(df), 3)[..., 2]
...
def plot_subplot(ax, y_train, y_test, y_train_pred, y_test_pred, title,
    ...
    ax.set_xlabel("Power (kW)", fontsize=font_size, fontfamily='Arial', labelpad=2)
    ax.set_ylabel("Prediction power (kW)", fontsize=font_size, fontfamily='Arial', labelpad=2)
...
if __name__ == '__main__':
    data_paths = [
        ("NV", r"D:\PY_Project\Paper1\Paper1_data\MPCVD_NV.csv"),
        ("SiV", r"D:\PY_Project\Paper1\Paper1_data\MPCVD_SiV.csv"),
        ("GeV", r"D:\PY_Project\Paper1\Paper1_data\MPCVD_GeV.csv"),
        ("SnV", r"D:\PY_Project\Paper1\Paper1_data\MPCVD_SnV.csv")
    ]
    ...
    plt.tight_layout()
    plt.savefig(r'D:\PY_Project\Paper1\Paper1_fig\MPCVD_DTR_XGB_compare.svg', format='svg', transparent=True, bbox_inches='tight')
    plt.show()