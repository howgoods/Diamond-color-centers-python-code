...
def load_and_split_data(path, test_size=0.3, seed=42):
    ...
    X_raw = np.around(np.array(df), 5)[..., 0:4]  
    y_raw = np.around(np.array(df), 5)[..., 4]    
...
if __name__ == '__main__':
    data_paths = [
        ("NV", r"D:\PY_Project\Paper1\Paper1_data\EI_NV_DWF.csv"),
        ("SiV", r"D:\PY_Project\Paper1\Paper1_data\EI_SiV_DWF.csv"),
        ("GeV", r"D:\PY_Project\Paper1\Paper1_data\EI_GeV_DWF.csv"),
    ]
    ...
    for i, (name, path) in enumerate(data_paths):
        ...
        plot_subplot(
            axes[0, i],
            y_train=y_train_true[::10],
            y_test=y_test_true[::10],
            y_train_pred=y_train_pred_true[::10],
            y_test_pred=y_test_pred_true[::10],
            title=titles[i],
    ...
    plt.tight_layout()
    plt.savefig(r'D:\PY_Project\Paper1\Paper1_fig\EI_DTR_XGB_DWF_compare.svg', format='svg', transparent=True, bbox_inches='tight')
    plt.show()