...
def load_and_split_data(path, test_size=0.3, seed=42):
    df = pd.read_csv(path)
    X = np.around(np.array(df), 5)[..., 0:3]
    y = np.around(np.array(df), 5)[..., 3]
...
def plot_colored_by_distance(y_true, y_pred, cmap='', label='', show_colorbar=False, ax=None, marker=''):
    error_scale = 0.06 * (np.max(y_true) - np.min(y_true))
...
def plot_subplot(ax, y_train, y_test, y_train_pred, y_test_pred, title,
    ...
    error_scale = 0.06 * (max_val - min_val)
    ...
    ax.text(0.66, 0.22, r"$R^2=$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='k')
    ax.text(0.82, 0.22, f"${r2_train:.3f}$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='grey')
    ax.text(0.84, 0.22, '/', transform=ax.transAxes, ha='right', fontsize=font_size - 1, color='k')
    ax.text(0.98, 0.22, f"${r2_test:.3f}$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='#df3881')

    ax.text(0.59, 0.16, r"$MSE=$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='k')
    ax.text(0.78, 0.16, f"${mse_train:.4f}$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='grey')
    ax.text(0.80, 0.16, '/', transform=ax.transAxes, ha='right', fontsize=font_size - 1, color='k')
    ax.text(0.98, 0.16, f"${mse_test:.4f}$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='#df3881')

    ax.text(0.59, 0.10, r"$MAE=$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='k')
    ax.text(0.78, 0.10, f"${mae_train:.4f}$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='grey')
    ax.text(0.80, 0.10, '/', transform=ax.transAxes, ha='right', fontsize=font_size - 1, color='k')
...
if __name__ == '__main__':
    data_paths = [
        ("NV", r"D:\PY_Project\Paper1\Paper1_data\EI_NV.csv"),
        ("SiV", r"D:\PY_Project\Paper1\Paper1_data\EI_SiV.csv"),
        ("GeV", r"D:\PY_Project\Paper1\Paper1_data\EI_GeV.csv"),
    ]

    xgb_params = build_xgb_model()

    fig, axes = plt.subplots(2, 3, figsize=(22, 11))  # 2行3列
    label_chars = ['(a)', '(b)', '(c)', '(d)', '(e)', '(f)']
    titles = ['NV color centers', 'SiV color centers', 'GeV color centers']
    font_size = 18
    bwith = 2
    ...
    for i, (name, path) in enumerate(data_paths):
        ...
        plot_subplot(axes[0, i],
                     y_train=result_tree['y_train'],
                     y_test=result_tree['y_test'],
                     y_train_pred=result_tree['y_train_pred'],
                     y_test_pred=result_tree['y_test_pred'],
                     title=titles[i],
    ...
    plt.tight_layout()
    plt.savefig(r'D:\PY_Project\Paper1\Paper1_fig\EI_DTR_XGB_compare.svg', format='svg', transparent=True, bbox_inches='tight')
    plt.show()