...
def evaluate_xgb(X_train, y_train, X_test, y_test, params):
    ...
    model = xgb.train(params=params, dtrain=dtrain, num_boost_round=2000, early_stopping_rounds=500, evals=evals, verbose_eval=False)
...
def plot_colored_by_distance(y_true, y_pred, cmap='', label='', show_colorbar=False, ax=None, marker=''):
    ...
    error_scale = 0.1 * (np.max(y_true) - np.min(y_true))

    # 沿 Y=X 方向加正向扰动（对 x 和 y 都加相同 delta）
    delta = np.random.uniform(0, jitter_scale, size=len(y_true))
...
def plot_subplot(ax, y_train, y_test, y_train_pred, y_test_pred, title,
    ...
    error_scale = 0.1 * (max_val - min_val)
    ...
    ax.text(0.64, 0.22, r"$R^2=$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='k')
    ax.text(0.80, 0.22, f"${r2_train:.3f}$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='grey')
    ax.text(0.82, 0.22, '/', transform=ax.transAxes, ha='right', fontsize=font_size - 1, color='k')
    ax.text(0.98, 0.22, f"${r2_test:.3f}$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='#df3881')

    ax.text(0.57, 0.16, r"$MSE=$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='k')
    ax.text(0.76, 0.16, f"${mse_train:.4f}$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='grey')
    ax.text(0.78, 0.16, '/', transform=ax.transAxes, ha='right', fontsize=font_size - 1, color='k')
    ax.text(0.98, 0.16, f"${mse_test:.4f}$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='#df3881')

    ax.text(0.57, 0.10, r"$MAE=$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='k')
    ax.text(0.76, 0.10, f"${mae_train:.4f}$", transform=ax.transAxes, ha='right', fontsize=font_size - 2, color='grey')
    ax.text(0.78, 0.10, '/', transform=ax.transAxes, ha='right', fontsize=font_size - 1, color='k')
...
if __name__ == '__main__':
    data_paths = [
        ("NV", r"D:\PY_Project\Paper1\Paper1_data\II_NV.csv"),
        ("SiV", r"D:\PY_Project\Paper1\Paper1_data\II_SiV.csv"),
        ("GeV", r"D:\PY_Project\Paper1\Paper1_data\II_GeV.csv"),
        ("SnV", r"D:\PY_Project\Paper1\Paper1_data\II_SnV.csv")
    ]
    ...
    plt.tight_layout()
    plt.savefig(r'D:\PY_Project\Paper1\Paper1_fig\II_DTR_XGB_compare.svg', format='svg', transparent=True, bbox_inches='tight')
    plt.show()